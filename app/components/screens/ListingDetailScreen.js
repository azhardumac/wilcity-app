...
class ListingDetailScreen extends PureComponent {
  ...
  _handleInbox = async () => {
    ...
    if (myID.toString() !== userID.toString()) {
      ...
      Alert.alert(translations.messageWarning);
    }
  };
  ...
  _handleReviewsLoadmore = async () => {
    const { navigation, listingReviewsAll, getListingReviewsLoadmore } =
      this.props;
    const { id } = navigation.state.params;
  ...
  renderActions = () => {
    ...
    const isChat = _.get(listingDetail, `${listingID}.isChat`, true);
    const phoneNumber = this._getPhoneNumber();
    const _column = (function () {
      if ((isReport && !isChat) || (isChat && !isReport)) {
        return 4;
      }
      if (isReport && isChat) {
        return 5;
  ...
  renderListFeatures = (type) => (id, item, max) => {
    ...
    const postType =
      get(navigation, "state.params.postType") ||
      get(listingDetail, `${id}.oHomeSections`.postType, {});
    return (
      <ListingListFeatureContainer
        key={item.key}
        params={{ id, item, max }}
  ...
  render() {
    const { navigation, settings, listingDetail, auth, translations } =
      this.props;
    const { params } = navigation.state;
    const id = `${params.id}_details`;
    const imageCover =
